...
spellchecker:
        ...
        ble $t3, $s6, punctuationcheck # so check if it is the right combination of punctuations and placed appropriately
...
punctuationcheck:
        ...
        beq $t3, 32, print # if it is a space, print anyway
        j checknext # check the next character in the token
        
        checknext: # check token_c_idx + 1
        	...
        	mul $t2, $s0, $t0
                ...
                # load tokens[tokens_number][token_c_index + 1] in $t9
                lb $t9, tokens($t2)
                
                beqz $t9, proceed # If there is only one occurence of the punctuation mark, continue (i.e. just a , or . or ! or ? alone)
                j maybeellipsis # if there is another punctuation next to it (hence not a null character), check if it is an ellipsis (...)
                
        maybeellipsis:
        
                bne $t3, '.', printwrong # if [tokens_number][token_c_index] is not a dot, then it could never be an ellipsis
                beq $t9, '.', checknextdot # then check the character next to it
                ...
                checknextdot:
                        ...
                        # load tokens[tokens_number][token_c_index + 1] in $t9
                        lb $t9, tokens($t2)
                        beq $t9, '.', checknextdot2 # if $t9 is a dot, then check the character next to it again (there are ONLY three dots in an ellipsis)
                        j printwrong # if it is not a dot, then it is wrong
                        ...
                        checknextdot2:
                                ...
                                # load tokens[tokens_number][token_c_index + 1] in $t9
                                lb $t9, tokens($t2)
                                # if $t9 is a dot, that means we have 4 occurences of dots '....' which is NOT an ellipsis
                                beq $t9, '.', printwrong # therefore it is wrong
                                j proceed # but if we have exactly 3 occurences of dots '...', check if the placement of the ellipsis in the sentence is correct
                                
                        
        # checks if the placement of the punctuations (',', '.', '!', '?', '...') in the sentence is correct
        proceed:
        	move $t6, $0 # set $t6 to null
                move $t1, $0 # set $t1 to null (token_c_idx = 0)
                add $t6, $t0, $zero  # add the contents of $t0 (tokens_number) to $t6 so we can manipulate it without changing the value of $t0 
                
                addi $t6, $t6, 1 # tokens_number + 1 (move to the next token) 
                
                mul $t2, $s0, $t6
                add $t2, $t2, $t1
                
                # load tokens[tokens_number + 1][token_c_index] in $t7 
                lb $t7, tokens($t2)
                beq $t7, ' ', checkbefore # only spaces should be found after any punctuation mark
                beq $t7, $0, checkbefore # or a null character (reached the end of the sentence)
                # so continue to check what character is present before the punctuation
                j printwrong # anything else is wrong
        ...
        checkbefore:
                ...
                # only alphabets or null characters should be present before a punctuation mark, nothing else
                beq $t7, ' ', printwrong
                beq $t7, '.', printwrong
                beq $t7, '!', printwrong
                beq $t7, '?', printwrong
